...
from Remote_User.models import ClientRegister_Model,earthquake_early_warning_prediction,detection_ratio,detection_accuracy
...
def View_Prediction_Of_Earthquake_Early_Warning_Ratio(request):
    ...
    ratio = ""
    kword = 'Earthquake Warning'
    print(kword)
    obj = earthquake_early_warning_prediction.objects.all().filter(Q(Prediction=kword))
    obj1 = earthquake_early_warning_prediction.objects.all()
    count = obj.count();
    count1 = obj1.count();
    ...
    ratio12 = ""
    kword12 = 'Explosion Warning'
    print(kword12)
    obj12 = earthquake_early_warning_prediction.objects.all().filter(Q(Prediction=kword12))
    obj112 = earthquake_early_warning_prediction.objects.all()
    count12 = obj12.count();
    ...
    return render(request, 'SProvider/View_Prediction_Of_Earthquake_Early_Warning_Ratio.html', {'objs': obj})
...
def View_Prediction_Of_Earthquake_Early_Warning(request):
    ...
    return render(request, 'SProvider/View_Prediction_Of_Earthquake_Early_Warning.html', {'list_objects': obj})
...
def Download_Trained_DataSets(request):
    ...
    for my_row in data:
        ...
        ws.write(row_num, 14, my_row.horizontalError, font_style)
        ws.write(row_num, 15, my_row.depthError, font_style)
        ws.write(row_num, 16, my_row.magError, font_style)
